...
function Tag({ label, type }: { label: string; type: 'veeqo' | 'amazon' | 'ebay' | 'sheets' }) {
  ...
  return (
    <span style={{
      ...styles[type], fontSize: 8, letterSpacing: '0.1em', padding: '2px 6px',
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div style={{ background: '#0f1419', border: '1px solid #1e2530', borderRadius: 6, padding: '12px 14px', height: '100%', overflow: 'hidden' }} className={className}>
      {children}
    </div>
  )
}
...
function MetricBlock({ label, value, sub, subColour }: { label: string; value: string; sub?: string; subColour?: string }) {
  return (
    <div>
      <div style={{ fontSize: 9, letterSpacing: '0.15em', color: '#475569', textTransform: 'uppercase', marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 26, fontWeight: 700, color: '#f1f5f9', lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 10, color: subColour || '#475569', marginTop: 4 }}>{sub}</div>}
    </div>
  )
}
...
function ProgressBar({ pct, colour }: { pct: number; colour: string }) {
  return (
    <div style={{ flex: 1, height: 7, background: '#1e2530', borderRadius: 3, overflow: 'hidden' }}>
      <div style={{ width: `${Math.min(100, pct)}%`, height: '100%', background: colour, borderRadius: 3, transition: 'width 0.6s ease' }} />
    </div>
  )
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 9, letterSpacing: '0.22em', color: '#475569', textTransform: 'uppercase', marginBottom: 8 }}>{children}</div>
}
    ...
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 9, color: '#475569', letterSpacing: '0.1em' }}>SHIFT ENDS</div>
          <div style={{ fontSize: 13, color: '#38bdf8', fontWeight: 700 }}>17:30</div>
        </div>
      </div>
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <span style={{ fontSize: 9, color: '#475569' }}>08:00</span>
            <span style={{ fontSize: 9, color: '#475569' }}>Now</span>
            <span style={{ fontSize: 9, color: '#334155' }}>17:30</span>
          </div>
        </>
      )}
    </Card>
      ...
      {loading ? (
        ...
        <>
          {[
            ...
            <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #111820' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: r.colour }}>{r.value}</span>
            </div>
          ))}
          ...
          {(data?.stock.lowItems?.length ?? 0) > 0 && (
            <details style={{ marginTop: 10 }}>
              <summary style={{ fontSize: 9, color: '#475569', cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase' }}>View low stock items</summary>
              <div style={{ marginTop: 6 }}>
                {data!.stock.lowItems.map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#64748b', padding: '3px 0', borderBottom: '1px solid #111820' }}>
        ...
        <>
          ...
          <div style={{ borderTop: '1px solid #1e2530', paddingTop: 10 }}>
            {[
              ...
              <div key={ch.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #111820', fontSize: 11 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10 }}>
            <div>
              <div style={{ fontSize: 9, color: '#475569', marginBottom: 2 }}>CANCELLED</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#f87171' }}>{data?.returns.cancelled ?? '--'}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 9, color: '#475569', marginBottom: 2 }}>SELLER RATING</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#f1f5f9' }}>{data?.rating.score ?? '--'}%</div>
            </div>
          </div>
        </>
      )}
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #1e2530', paddingTop: 10 }}>
            <div>
              <div style={{ fontSize: 9, color: '#475569', marginBottom: 2 }}>RETURNS</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#f87171' }}>{data?.returns.returns ?? '--'}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, color: '#475569', marginBottom: 2 }}>CANCELLED</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#f87171' }}>{data?.returns.cancelled ?? '--'}</div>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 9, color: '#475569', marginBottom: 2 }}>FEEDBACK</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#22c55e' }}>{data?.rating.score ?? '--'}%</div>
            </div>
          </div>
        </>
      )}
          ...
          {data.metrics.map((m, i) => (
            <div key={i} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: 10, color: '#64748b' }}>{m.metric}</span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 10, color: m.pct >= 90 ? '#22c55e' : m.pct >= 70 ? '#f59e0b' : '#f87171', width: 32, textAlign: 'right', flexShrink: 0 }}>{m.pct}%</span>
              </div>
            </div>
          ))}
          {data.lastSync && (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            <thead>
              <tr>{['Channel','Returns','Cancels'].map(h => (
                <th key={h} style={{ fontSize: 9, letterSpacing: '0.1em', color: '#475569', textTransform: 'uppercase', textAlign: 'left', paddingBottom: 8, borderBottom: '1px solid #1e2530' }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.ch}>
                  <td style={{ padding: '6px 0', borderBottom: '1px solid #111820', color: '#94a3b8' }}>{r.ch}</td>
                  <td style={{ padding: '6px 0', borderBottom: '1px solid #111820', color: r.returns > 5 ? '#f87171' : '#94a3b8' }}>{r.returns}</td>
                  <td style={{ padding: '6px 0', borderBottom: '1px solid #111820', color: r.cancels > 5 ? '#f87171' : '#e2e8f0', textAlign: 'right' }}>{r.cancels}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10 }}>
            <span style={{ fontSize: 10, color: '#64748b' }}>Return rate</span>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#f59e0b' }}>{returnRate}{returnRate !== '--' ? '%' : ''}</span>
          </div>
        </>
      )}
    </Card>
...
const defaultLayouts: Layouts = {
  lg: [
    { i: 'veeqo-orders', x: 0, y: 0, w: 3, h: 4 },
    { i: 'veeqo-shift',  x: 3, y: 0, w: 5, h: 7 },
    { i: 'amazon',       x: 8, y: 0, w: 4, h: 7 },
    { i: 'veeqo-stock',  x: 0, y: 4, w: 3, h: 7 },
...
export default function Dashboard() {
  ...
  const rangeLabels: Record<DateRange, string> = { today: 'Today', yesterday: 'Yesterday', '7days': '7 Days', '30days': '30 Days' }

  return (
    <div style={{ minHeight: '100vh', background: '#0a0c0f', padding: 16 }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid #1e2530' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => fetchAll(range)} style={{ fontSize: 9, padding: '4px 10px', background: '#1e2530', border: '1px solid #2a3a50', color: '#38bdf8', borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            ↻ Refresh
          </button>
          <button onClick={resetLayout} style={{ fontSize: 9, padding: '4px 10px', background: 'transparent', border: '1px solid #1e2530', color: '#475569', borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            Reset Layout
          </button>
          <span style={{ fontSize: 9, color: '#334155' }}>Last: {lastRefresh}</span>
        </div>
      </div>

      {/* Tip bar */}
      <div style={{ fontSize: 9, color: '#334155', marginBottom: 12, letterSpacing: '0.08em' }}>
